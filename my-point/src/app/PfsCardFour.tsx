...
const PfsCardFour: React.FC<Props> = ({
  ...
  const handleChangePaymentFee = (e: React.ChangeEvent<HTMLInputElement>) => {
    const inputNumber = Number(e.target.value.replace(/[^0-9.-]+/g, '')); // Remove non-numeric characters
    const formattedPaymentFee = inputNumber.toLocaleString(); // Format the input value with commas
  ...
  const handleChangePersenPaymentFee = (
  ...
  return (
    ...
    >
      <div className="flex flex-col md:flex-row justify-between items-center mb-6">
        <div>
          <h2 className="text-sm font-semibold mb-2">SYSTEM FEE</h2>
        </div>
      ...
      <div className="flex flex-col md:flex-row justify-between mb-4 gap-2">
          ...
          <PfsMainInputOne
            ...
            label2={
              <p className="text-center">
                Please update by yourself, for example 10% or what percentage of the payment section
              </p>
            }
          />
        ...
        <PfsIconInputOne
          ...
          label2={
            <p className="text-center">
              Please fill in, for example ¥330 or how much
            </p>
          }
          disabled={!isEditing}
        />
